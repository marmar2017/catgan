...
class CatGANInstructor(BasicInstructor):
    ...
    def _run(self):
        ...
        if False:
            for i, (parent, parent_opt) in enumerate(zip(self.parents, self.parent_mle_opts)):
                ...
                self.pretrain_generator(cfg.MLE_train_epoch)  # pre train the generator
        ...
        # load model from pretrained model path
            ...
            print('load model from pretrained model path...')
            #model_path = '/home/user1/Ru_experiement/TextGAN-PyTorch/save/20210325/sr/catgan_vanilla_dt-Ra_lt-ragan_mt-ra+rs_et-Ra_sl70_temp100_lfd0.001_T0325_1613_40/models/gen_MLE_00090.pt'
            # model_path = '/home/user1/Ru_experiement/TextGAN-PyTorch/save/20210327/sr250/catgan_vanilla_dt-Ra_lt-ragan_mt-ra+rs_et-Ra_sl63_temp100_lfd0.001_T0327_1018_46/models/gen_MLE_00100.pt'
            model_path = '/home/user1/Ru_experiement/TextGAN-PyTorch/save/20210412/amazon_app_movie/catgan_vanilla_dt-Ra_lt-ragan_mt-ra+rs_et-Ra_sl41_temp100_lfd0.001_T0412_1517_22/models/gen_MLE_00100.pt'
            model_path = '/home/user1/Ru_experiement/TextGAN-PyTorch/save/20210331/cr150/catgan_vanilla_dt-Ra_lt-ragan_mt-ra+rs_et-Ra_sl37_temp100_lfd0.001_T0331_1910_23/models/gen_ADV_00090.pt'
            self.gen.load_state_dict(torch.load(model_path,map_location='cpu'))
            #self.gen.load_state_dict(torch.load('/home/user1/Ru_experiement/TextGAN-PyTorch/save/20210330/cr150/catgan_vanilla_dt-Ra_lt-ragan_mt-ra+rs_et-Ra_sl37_temp100_lfd0.001_T0330_1633_53/models/gen_MLE_00130.pt',map_location='cpu'))
            self.gen.eval()
            # don't forget to saving state_dict to parents
            self.parents[0] = copy.deepcopy(self.gen.state_dict())
        ...
        for adv_epoch in progress:
            ...
            # ===Test===
            # adv_log_step determines when to save model and output log every 20 epoch would ouput log and save model
            # changed to every 10 epochs
    ...
    def prepare_train_data(self, which, step=None):
        ...
        real_samples_list = [
            ...
            for i in range(cfg.k_label)]
        if cfg.CUDA:
            self.real_samples_list = [self.real_samples_list[i].cuda() for i in range(cfg.k_label)]
        if which == 'D':
